			  ...
			  struct ispccdc_update_config *ccdc_struct)
{
	...
	if (ISP_ABS_CCDC_ALAW & ccdc_struct->update) {
		ccdc->alaw = !!(ISP_ABS_CCDC_ALAW & ccdc_struct->flag);
		ccdc->update |= ISP_ABS_CCDC_ALAW;
	}

	if (ISP_ABS_CCDC_LPF & ccdc_struct->update) {
		ccdc->lpf = !!(ISP_ABS_CCDC_LPF & ccdc_struct->flag);
		ccdc->update |= ISP_ABS_CCDC_LPF;
	}
	...
	if (ISP_ABS_CCDC_BLCLAMP & ccdc_struct->update) {
		...
		ccdc->obclamp = !!(ISP_ABS_CCDC_BLCLAMP & ccdc_struct->flag);
		ccdc->update |= ISP_ABS_CCDC_BLCLAMP;
	}
	...
	if (ISP_ABS_CCDC_BCOMP & ccdc_struct->update) {
		...
		ccdc->update |= ISP_ABS_CCDC_BCOMP;
	}
	...
	if (ISP_ABS_CCDC_FPC & ccdc_struct->update) {
		...
		ccdc->fpc_en = !!(ISP_ABS_CCDC_FPC & ccdc_struct->flag);
...
{
	if (ccdc->update & ISP_ABS_CCDC_ALAW) {
		ispccdc_configure_alaw(ccdc);
		ccdc->update &= ~ISP_ABS_CCDC_ALAW;
	}

	if (ccdc->update & ISP_ABS_CCDC_LPF) {
		ispccdc_configure_lpf(ccdc);
		ccdc->update &= ~ISP_ABS_CCDC_LPF;
	}

	if (ccdc->update & ISP_ABS_CCDC_BLCLAMP) {
		ispccdc_configure_clamp(ccdc);
		ccdc->update &= ~ISP_ABS_CCDC_BLCLAMP;
	}

	if (ccdc->update & ISP_ABS_CCDC_BCOMP) {
...
void ispccdc_restore_context(struct isp_device *isp)
{
	...
	ccdc->update = ISP_ABS_CCDC_ALAW | ISP_ABS_CCDC_LPF
		     | ISP_ABS_CCDC_BLCLAMP | ISP_ABS_CCDC_BCOMP;
