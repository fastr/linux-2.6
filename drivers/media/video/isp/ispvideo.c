 ...
 * @video: ISP video object
 * @discard: If not 0, request video queue layer to discard DONE buffers.
 *
 * This function is intended to be used on suspend/resume scenario. It
 * requests video queue layer to discard buffers marked as DONE if necessary
 * and requests ISP modules to queue again the ACTIVE buffer if there's any.
 */
void isp_video_resume(struct isp_video *video, int discard)
{
	struct isp_buffer *buf = NULL;

	if (discard && video->type == V4L2_BUF_TYPE_VIDEO_CAPTURE)
